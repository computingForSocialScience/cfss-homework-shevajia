...
f_exp = open("cadre_info_list.csv", 'r', encoding = 'utf8')

info= []
for row in f_exp:
    items = row.split(u',')
    id = items[0]
    professional_title = items[11]
    party = items[14]
    notes = items[-1]
    info.append((id, professional_title, party, notes))    

info_new = []
for row in info:
    CCP = None
    if row[0] == u'\ufeffid':
        row_new = (row[0], row[1], row[2], 'military', 'CCP')
        info_new.append(row_new)
    else: 
        if row[1] == u'少將' or row[1] == u'中將' or row[1] == u'上將':
            military = True
        else:
            military = False #determines whether the person belongs to the military

        if row[2] == u'非中國共產黨':
            match = re.search(u'開除', row[3])
            if match:
                pass
            else:
                CCP = False
